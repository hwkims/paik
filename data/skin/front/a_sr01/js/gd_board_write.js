...
$(document).ready(function () {
    ...
    $("#frmWrite").validate({
        submitHandler: function (form) {
            ...
            if($('[name="bdCategoryFl"]').val()=="y" && ($('[name="category"]').val()=="")){
				alert("말머리를 선택해 주세요.");
				return false;
			}
			...
			if($('[name="bdId"]').val()=="goodsinforeview"){
				var _content = $('#editor').val();
				_content = _content.replace(/<(?!img).*?>/ig, '').replace('&nbsp;', '');
				...
				if(_content.indexOf("<img src=")>=0 || $('[name="upfiles[]"]').val()!=""){
					$('[name="writeType"]').val('image');
				}
			}			
            form.submit();
