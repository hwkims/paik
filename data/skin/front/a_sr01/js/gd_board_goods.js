...
$(document).ready(function () {
	...
	$('body').delegate('.js_data_row2', 'click', function () {
        var bdId = $(this).closest('.js_data_row2').data('bdid');    //전역변수등록
        var sno = $(this).closest('.js_data_row2').data('sno');  //전역변수등록
        var auth = $(this).closest('.js_data_row2').data('auth');
        var notice = $(this).closest('.js_data_row2').data('notice');
        var domRoot = domSelect(bdId, sno, notice);
    ...
    $('body').delegate('.js_btn_modify', 'click', function () {
        ...
        function gd_modify_write() {
			...
			if(location.href.indexOf('goodsinfo')>0){
				url="../board/popup_goodsinfo_board_write.php?mode=modify&bdId="+bdId+"&sno="+sno+"&oldPassword="+passwordLayer.value();
			}
...
function goGoodsAjaxPage(bdId, queryStirng) {
    ...
    else if (bdId == 'goodsinforeview') {
        ajaxDataEl = $('#ajax-goods-goodsinforeview-list');
    }
    else {
...
function gd_open_write_popup(bdId, goodsNo,orderGoodsNo) {
    ...
    window.open(url,'review','width=900,height=850,left=300,scrollbars=yes');
}
